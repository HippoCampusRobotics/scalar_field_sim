...
from scalar_field_sim.markers import make_source_markers, make_wall_markers
...
class FieldVisualizationNode(Node):
    ...
    def __init__(self):
        ...
        # Smaller values increase visual resolution but also increase the number
        # of published points.
        ...
        # - "height": scalar value is also shown as height -> helpful for
        # debugging
        ...
        # Constant vertical shift applied to the full cloud for easier
        # separation from other visualizations such as the belief
        # mean/variance clouds.
        ...
        self.get_logger().info(
            f'Using grid_step = {self._grid_step} '
            + 'for visualization of ground truth'
        )

        self._z_mode = (
            self.get_parameter('z_mode').get_parameter_value().string_value
        )
        self._height_scale = (
            self.get_parameter('height_scale')
            .get_parameter_value()
            .double_value
        )
        self._publish_sources = (
            self.get_parameter('publish_sources')
            .get_parameter_value()
            .bool_value
        )
    ...
    def _on_set_parameters(self, params) -> SetParametersResult:
        ...
        If a valid update is accepted,
        the node republishes the ground-truth cloud.
        """
    ...
    def _publish_field_visualization(self) -> None:
        ...
        """
        positions, values = self.field.evaluate_on_grid(
