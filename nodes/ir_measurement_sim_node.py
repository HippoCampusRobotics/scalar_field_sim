...
class IrMeasurementSimNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self._measurement_topic = str(
            self.get_parameter('measurement_topic').value
        )
        self._trigger_service_name = str(
            self.get_parameter('trigger_service_name').value
        )
        self._sample_service_name = str(
            self.get_parameter('sample_service_name').value
        )
        self._frame_id_fallback = str(
            self.get_parameter('frame_id_fallback').value
        )
    ...
    async def _handle_trigger(
        ...
        self.get_logger().debug(
            ...
            f'({odom.pose.pose.position.x:.3f}, '
        ...
        req.query.header.frame_id = (
            odom.header.frame_id or self._frame_id_fallback
        )
        ...
        request_dt_s = (
            self.get_clock().now() - request_start
        ).nanoseconds * 1e-9
        ...
        if latest_odom_after is not None:
            ...
            dz = (
                latest_odom_after.pose.pose.position.z
        ...
        if (
            measurement.header.stamp.sec == 0
            and measurement.header.stamp.nanosec == 0
        ):
        ...
        self.get_logger().debug(
            ...
            f'({measurement.pose.position.x:.3f}, '
            f'{measurement.pose.position.y:.3f}, '
            f'{measurement.pose.position.z:.3f}) '
...
def main(args=None) -> None:
    ...
    # A multithreaded executor is used because this node handles one
    # service call while also waiting for the response of another service call.
